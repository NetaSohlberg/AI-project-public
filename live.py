'''
Yeynit Asraf 207946146
Shira Shwartz 211485198
Neta Sohlberg 322217225
'''
...
'''
Live file- run it to start the program.
'''
...
# play the camera and call the other functions to recognize signs of distress in the baby
def mainFunc():
    cap = cv2.VideoCapture(0)   # read video from camera
    ...
    frame1=frame1[1]
    time.sleep(5.0)  # wait 5 seconds until good camera operation

    first_color = color_frame1(frame1)  # save the mean color of the face in the first frame

    while (True):   # while the camera play
        start_time = time.time()
        frame2 = cap.read() # read the current frame
        frame2= frame2[1]
        ...
        cv2.imshow('window', frame2)    # show the video
...
# play the program when the user click on the start button
